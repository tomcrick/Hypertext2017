...
from language settings in users' profiles and those for statuses.The
first step is to construct two graphs to depict relationships between
language of users and languages used in posts.  Then, using graphs
properties, we will conduct the analysis of language communities and
their diversity and if it is effected by the nature of topic. The
study will use only original statuses (tweet) as the basic entity as
we are not measuring reactions.
...
other is based on language communities. For generating necessary
graphs, we only need three entities from each status, '\emph{user
id}', '\emph{user language setting}', and '\emph{status
language}'. Those values can be extracted from [status]['user']['id'],
[status]['user']['lang'], and [status]['lang'], respectively. In the
...
types; users and posting language. Each posted tweet results in two
...
node represents language of the tweet. Edges link user with the
...
language community, as shown in
Figure~\ref{fig:graphstructures}.b. For each user node we generate one
node from the `{\emph{\{profile\_lang:xx\}}}' attribute, and another
node from the posting language it connects to. Theorytically, each
posted tweet results in two nodes, one for the user profile, and the
other for the posting language. In our example above, users with `fr'
...
From the users graph, we generated two communities graphs. 
Figure \ref{fig:communitiesgraphs}.a depicts all language communities together, 
while \ref{fig:communitiesgraphs}.b visualises a filtered graph of 
active multilingual communities only and how they relate to each other. 
...
in other language communities. Edges link between \emph{profile} and 
...
The filtered graph, was produced in the following manner.
First, in the users graph we eleminated users with activiy lower 
than the overall average (5 tweets/user), and generated communities graph from 
the rest. Then, for the resulted graph, we removed all 'self-loop' edges. Afterward, 
we measured the \emph{weighted indegrees} and \emph{weighted outdegrees}, 
and eleminated those communities scored low in both measures. Finally, we measured 
the graph again based on interactions amongst those communities, and plotted the 
graph as shown in fig~\ref{fig:communitiesgraphs}.b.
