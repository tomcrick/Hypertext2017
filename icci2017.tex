...
shows these groups, their users and activities. As we can see that about 85\% of users are monoglingual,
and their activity is 47\% of all tweets. Additionally, while the average acitivity of users is 5 posts
per user, monolingual users were the least active ones, scoring an average of 2 tweets per user.
...
From users graph, we can generate any number of communities graphs. 
From those 20 multilingual groups, we generated two interesting communities graphs, 
monolingual and multilingual communities. As we have 85?\% users from monolingual 
users only, we sat a qualifying threshold of outdegree 1. We generated two 
communities graphs, one to examine how language communities 
...
of these group in terms of their we can see those groups. They formed about 85\% of users, however in terms of activity, those 
users achieved less than 50\% from all tweets. The overall activity ratio is 5 tweets 
per user, however, monolingual users scored 2, placing this group
as the least active group of users. From this we can see that multilingual 
users are more active than monolingual users. We also found that the correlation
between outdegree the liklihood of posting in a different language is 0.68.

\subsection{Profile and Posting Communities}\label{eurovisionpostinglangs}

This graph tells us about community language sizes, multilinguals amongst 
users and their activity. The results show that the 
...
with the default value. As might be expected, English
was the most frequently-used posting language, while 4\% statuses
could not be categorised.We found that these three measures 
(i.e. profile community size, profile activity, and frequency of 
language uses) are highly correlated. 
...
22 and 25 different languages. Although we
