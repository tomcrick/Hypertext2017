...
As shown in Figure~\ref{fig:graphstructures}.a, nodes in this graph are 
of two types; users and posting language. Each posted tweet results in 
...
In the example above, the profile language setting for user '0123' is 'en', he 
posted 3 tweets in 'en' and 4 in 'ar'. It is important to note that user nodes 
...
This graph, as shown in Figure~\ref{fig:graphstructures}.b has one type of nodes, 
language community nodes. Theorytically, each posted tweet results in two nodes, 
one for the user profile, and the other for the posting language. In our example above, 
...
as 'ar' have posted 4 tweets in 'ar'. 

By utilising network graph measures, this graph can provide measures about 
language communities. Further, we can measure what profile languages are
present, what languages are used for posting, and how
different language communities are linked to each other, as we will see in the following sections.

\subsection{Measures}
As we are using network graphs in this study, this section presents the measures we use 
in the analysis for each graphs. It is important to recall that both graphs are directed.
For each generated graph, we will use 
